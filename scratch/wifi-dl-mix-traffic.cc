...
// ./waf --run "wifi-dl-mix-traffic --simulationTime=10 --mcs=11 --txopLimit=10016 --dlAckType=2 --channelWidth=20 --guardInterval=800 --radius=1"
...
WifiDlOfdmaExample::Setup (void)
{
  ...
  PacketSinkHelper packetSinkHelper1 (socketType, InetSocketAddress (Ipv4Address::GetAny (), m_port));
  m_sinkApps.Add(packetSinkHelper1.Install (m_staNodes.Get(2)));
  m_sinkApps.Add(packetSinkHelper1.Install (m_staNodes.Get(3)));

  PacketSinkHelper packetSinkHelper2 (socketType, InetSocketAddress (Ipv4Address::GetAny (), m_port + 1));
  m_sinkApps.Add(packetSinkHelper2.Install (m_staNodes.Get(4)));
  m_sinkApps.Add(packetSinkHelper2.Install (m_staNodes.Get(5)));
...
WifiDlOfdmaExample::EstablishBaAgreement (Mac48Address bssid)
{
  ...
  if ( m_currentSta >= 2 && m_currentSta <= 3 ) { // only for stations 2, 3 [ON/OFF]
  ...
  if ( m_currentSta > 3 ) { // Only for station 4 and 5 [BULK]
    ...
    // dest.SetTos (0xb8); //AC_VI
    InetSocketAddress dest (m_staInterfaces.GetAddress (m_currentSta), m_port + 1);
    client.SetAttribute ("Remote", AddressValue (dest));
    m_bulkSourceApps.Add(client.Install (m_apNodes));
    m_bulkSourceApps.Stop (Seconds (m_warmup + m_simulationTime));
    //Simulator::Schedule (Seconds (m_warmup), &WifiDlOfdmaExample::StartBulkClient, this, client);

    //uint64_t startTime = std::ceil (Simulator::Now ().ToDouble (Time::MS) / offInterval) * offInterval;

    //Simulator::Schedule (MilliSeconds (static_cast<uint64_t> (startTime) + 110) - Simulator::Now (), &WifiDlOfdmaExample::StartBulkClient, this, client);
  }
