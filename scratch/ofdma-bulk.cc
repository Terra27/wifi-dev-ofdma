...
WifiDlOfdmaExample::EstablishBaAgreement (Mac48Address bssid)
{
  ...
  BulkSendHelper client (socketType, dest);
  client.SetAttribute ("SendSize", UintegerValue (m_payloadSize));
  client.SetAttribute ("MaxBytes", UintegerValue (0));
  m_clientApps.Add (client.Install (m_apNodes));
  m_clientApps.Start(Seconds(m_warmup));
  m_clientApps.Stop (Seconds (m_warmup + m_simulationTime)); 
  ...
  //Simulator::Schedule (MilliSeconds (static_cast<uint64_t> (startTime) + 110) - Simulator::Now (), &WifiDlOfdmaExample::StartClient, this, client);
