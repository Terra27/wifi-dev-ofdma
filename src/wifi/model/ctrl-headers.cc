...
CtrlBAckResponseHeader::Print (std::ostream &os) const
{
  if (m_baType.m_variant != BlockAckType::MULTI_STA)
    {
      os << "TID_INFO=" << m_tidInfo << ", StartingSeq=" << std::hex << m_baInfo[0].m_startingSeq << std::dec;
    }
  else
    {
      for (std::size_t i = 0; i < m_baInfo.size (); i++)
        {
          os << "{AID=" << GetAid11 (i)
             << ", TID=" << GetTidInfo (i)
             << ", StartingSeq=" << std::hex << m_baInfo[i].m_startingSeq << std::dec << "}";
        }
    }
}
...
void
CtrlBAckResponseHeader::SetUnassociatedStaAddress (Mac48Address ra, std::size_t index)
{
  NS_ASSERT (m_baType.m_variant == BlockAckType::MULTI_STA && index < m_baInfo.size ());
  ...
  switch (m_baType.m_variant)
    {
      ...
      case BlockAckType::EXTENDED_COMPRESSED:
      case BlockAckType::MULTI_STA:
          for (auto& byte : m_baInfo[index].m_bitmap)
            {
              i.WriteU8 (byte);
            }
          break;
