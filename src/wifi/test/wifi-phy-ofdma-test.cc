...
TestUlOfdmaPowerControl::SendMuBar (std::vector <uint16_t> staIds)
{
  ...
  muBar.SetUlLength (WifiPhy::ConvertHeTbPpduDurationToLSigLength (MicroSeconds (120), DEFAULT_FREQUENCY));
  ...
  WifiTxVector txVector = WifiTxVector (WifiPhy::GetHeMcs7 (), 0, WIFI_PREAMBLE_HE_SU, 800, 1, 1, 0,
                                        DEFAULT_CHANNEL_WIDTH, false, false, m_bssColor);
  ...
  Time nav = m_apDev->GetMac ()->GetSifs ();
  nav += m_phyAp->CalculateTxDuration (GetBlockAckSize (BlockAckType::COMPRESSED), txVector, DEFAULT_FREQUENCY);
